...
main(int argc, char **argv)
{
    ...
    */
    ...
    {
        std::cout << "ERROR: document does not contain \"matlab\"!"
                  << std::endl;
        return 1;
    }
    if (!d["matlab"].IsBool())
    ...
    std::cout << "matlab = " << (d["matlab"].GetBool() ? "true" : "false")
              << std::endl;

    // get a vector
    if (!d.HasMember("grid.o"))
    {
        std::cout << "ERROR: document does not contain \"grid.o\"!"
                  << std::endl;
        return 1;
    }
    if (!d["grid.o"].IsArray())
