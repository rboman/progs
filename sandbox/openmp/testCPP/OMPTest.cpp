...
public:
    MatMult(int n, double *a, double *b, double *c, int nbt, int tmax)
        : my_n(n), my_a(a), my_b(b), my_c(c), my_nbt(nbt), my_tmax(tmax)
    {
    }
...
{
    out << obj.idx1 << '\t' << obj.idx2 << '\t' << obj.size << '\t' << obj.nbt
        << '\t' << obj.mem << '\t' << obj.cpu << '\t' << obj.flops << '\n';
    return out;
}
...
OMPTester::execute()
{
    ...
    for (int siz = srange.getMin(); siz <= srange.getMax();
         siz *= srange.getStep())
        ntest1++;
    for (int nbt = trange.getMin(); nbt <= trange.getMax();
         nbt += trange.getStep())
        ntest2++;
         ...
         siz *= srange.getStep())
    {
             ...
             nbt += trange.getStep())
        {
            ...
            OMPData res = OMPData(idx1, idx2, siz, nbt, test.getMem(), cpu,
                                  test.flops(nbt));
