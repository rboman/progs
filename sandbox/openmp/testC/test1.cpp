...
void getarg(int argc, char *argv[], int nn, int *val)
{
    if(nn>=argc) return;
...
*/
    ...
    {
        std::cerr << "not enough memory (" << (m*n*8.)/1024/1024 << "Mo)!\n";
        exit(1);
    }
    ...
    for(int nbt=1; nbt<=nbth; ++nbt)
    {
        ...
        double mflops=double(m)*double(n)*double(tmax1)*double(tmax2)/cpu[nbt-1]/1e6;

        std::cout << cpu[nbt-1] << '\t' << cpu[0]/cpu[nbt-1] << '\t' << cpu[0]/cpu[nbt-1]/nbt << '\t' << mflops << '\n';
    }
