...
*/
    ...
    for(it2=myMap.begin(); it2!=myMap.end(); ++it2)
    {
        ...
        while(myOff<off)
        {  
            fputc(fgetc(fileIn), fileOut); myOff++;
        }
        size_t i;
...
Converter::renameFiles() const
{
    ...
    if(err) printRenameError(fileNameIn,fileNameBak);

    err = rename(fileNameOut.c_str(), fileNameIn.c_str());
    if(err) printRenameError(fileNameOut,fileNameIn);
}

void
Converter::printRenameError(const std::string &file1, const std::string &file2) const
{
