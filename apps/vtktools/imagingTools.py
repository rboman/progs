...
def createSignedEuclideanDistanceMapEdge(segmentedImage):
    ...
    '''
    negative = generalTools.castImage(createNegative(generalTools.castImage(segmentedImage, coding='uchar')),coding='double')
    euclide1 = createEuclide(generalTools.castImage(segmentedImage, coding='double'))
    euclide2 = createEuclide(generalTools.castImage(negative, coding='double'))
    segmentedMask = multiplyByConstant(thresholdByUpper(segmentedImage,0.1,1.0,0), segmentedImage.GetSpacing()[0]/2);
    negativeMask = multiplyByConstant(thresholdByUpper(negative,0.1,1.0,0), segmentedImage.GetSpacing()[0]/2);
...
def createSignedEuclideanDistanceMap2D(segmentedImage):
    ...
    exterior = negative values giving distance to edge
    '''
    negative = createNegative(generalTools.castImage(segmentedImage, coding='uchar'))
    euclide1 = createEuclide2D(generalTools.castImage(segmentedImage, coding='double'))
    euclide2 = createEuclide2D(generalTools.castImage(negative, coding='double'))
...
def createSignedEuclideanDistanceMapEdge2D(segmentedImage):
    ...
    lx, ly, lz = distMap.GetDimensions()[0],distMap.GetDimensions()[1],distMap.GetDimensions()[2]  
    for k in range(lz):
        for j in range(ly):
            for i in range(lx):
                val = image1.GetScalarComponentAsDouble(i,j,k,0)-image2.GetScalarComponentAsDouble(i,j,k,0)
                if val > 0:
...
def changeExtent(image, extent):
    ...
    outExtent = [0,extent[1]-extent[0],0,extent[3]-extent[2],0,extent[5]-extent[4]]
...
def createVtkSegmImageFromImplicitFunction(origin, spacing, dimensions, func1, func2=None):
    ...
    lx, ly, lz = imageData.GetDimensions()[0],imageData.GetDimensions()[1],imageData.GetDimensions()[2]
    for k in range(lz):
        for j in range(ly):
            for i in range(lx): 
                pos = [imageData.GetOrigin()[0]+i*imageData.GetSpacing()[0],imageData.GetOrigin()[1]+j*imageData.GetSpacing()[1],imageData.GetOrigin()[2]+k*imageData.GetSpacing()[2]]
                if func2:
...
def linearInterpolation(image):
    ...
    outSpacing = [ image.GetSpacing()[0],image.GetSpacing()[1],image.GetSpacing()[0] ]
    outDimensions = [ image.GetDimensions()[0], image.GetDimensions()[1], int(floor( ( (image.GetDimensions()[2]-1)*image.GetSpacing()[2] ) / outSpacing[2] ))+1]
    ...
    lx, ly, lz = interpolatedImage.GetDimensions()[0],interpolatedImage.GetDimensions()[1],interpolatedImage.GetDimensions()[2]  
    if ( ( (image.GetDimensions()[2]-1) *  image.GetSpacing()[2]) == ( (outDimensions[2]-1) *  outSpacing[2]) ):
        lz = lz-1
        for j in range(ly):
            for i in range(lx):
                interpolatedImage.SetScalarComponentFromDouble(i,j,lz,0,image.GetScalarComponentAsDouble(i,j,image.GetDimensions()[2]-1,0))
    for k in range(lz):
        interpolatedSliceHeight = interpolatedImage.GetOrigin()[2] + k *  interpolatedImage.GetSpacing()[2]
        imageSliceDownIndice = int(floor((interpolatedSliceHeight - image.GetOrigin()[2])/image.GetSpacing()[2]))
        imageSliceUpIndice = imageSliceDownIndice+1
        imageSliceDownHeightDiff = interpolatedSliceHeight - (image.GetOrigin()[2] + imageSliceDownIndice * image.GetSpacing()[2])
        imageSliceUpHeightDiff = image.GetOrigin()[2] + imageSliceUpIndice * image.GetSpacing()[2] - interpolatedSliceHeight
        imageDistanceBetweenSlices = imageSliceDownHeightDiff + imageSliceUpHeightDiff
        for j in range(ly):
            for i in range(lx):
                d = (image.GetScalarComponentAsDouble(i,j,imageSliceDownIndice,0)*imageSliceUpHeightDiff+image.GetScalarComponentAsDouble(i,j,imageSliceUpIndice,0)*imageSliceDownHeightDiff)/imageDistanceBetweenSlices
...
def createTestImage():
    ...
    range1 = [image.GetDimensions()[0]/3, image.GetDimensions()[0]*2/3, image.GetDimensions()[1]/3, image.GetDimensions()[1]*2/3, image.GetDimensions()[2]/3, image.GetDimensions()[2]*2/3]
...
def extractContourPointsFromSegementedImage(image,th=(0,1), filename = "contourPoints"):
    ...
    file.write("# contours points created via extractContourPointsOfSegementedImage\n")
    for cid in range(n):
    ...
    for slicenbr in range( image.GetExtent()[4],image.GetExtent()[5] ):
        ...
        for cid in range(n):
            ...
            for pid in range(m):
                ...
                image.GetScalarComponentAsDouble( ijk[0]+1, ijk[1], ijk[2], 0) ,
                image.GetScalarComponentAsDouble( ijk[0], ijk[1]-1, ijk[2], 0) ,
                image.GetScalarComponentAsDouble( ijk[0], ijk[1]+1, ijk[2], 0) ,
        ...
        for iy in range( image.GetExtent()[2], image.GetExtent()[3]):
            for ix in range( image.GetExtent()[0], image.GetExtent()[1]): 
                ...
                if (((valsup == th) and (val == 0)) or ((valinf == th) and (val == 0))):
                    pos = [image.GetOrigin()[0]+ix*image.GetSpacing()[0],image.GetOrigin()[1]+iy*image.GetSpacing()[1],image.GetOrigin()[2]+iz*image.GetSpacing()[2] ]    
                    n = [0,0,0]
...
# BROUILLON !!!!!!!!!!!
    ...
    for i in range(0,len(th)):
        ...
        for slicenbr in range( image.GetExtent()[4],image.GetExtent()[5] ):
            ...
            skinExtractor.SetValue(0,th)        # extraction de tous les contours
            skinExtractor.Update()
            contour = skinExtractor.GetOutput()
