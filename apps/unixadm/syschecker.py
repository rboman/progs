...
#    . set EDITOR=vim
#    . crontab -e
#    . add the line "0 0 * * * (path)/rbcron.py >/dev/null 2>&1" 
#
# see http://www.debian-administration.org/articles/56

import os, sys, os.path, socket, smtplib
import time
...
class SysChecker:
    ...
    def __checkignorenice(self):
        ...
        if vali!=0:
            ...
            message+="OK!\n"
        self.__mailmsg(self.watchers, subject="checkignorenice()", text=message)

    def execute(self):
        self.report=""
        self.__checkignorenice()
        if self.report:
            self.__mailmsg(self.watchers, subject="SysChecker Report", text=self.report)
