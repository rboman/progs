...
MyWidgetI::MyWidgetI(QWidget *parent, const char *name, WFlags fl)
    : MyWidget(parent, name, fl)
{
    update();
}
...
void
MyWidgetI::updateTextLineEdit(QLineEdit *ledit, double val)
{
    QString s;
    s.setNum(val);
    ledit->setText(s);
...
void
MyWidgetI::tload_slot()
{
    tpar.load("mesh.txt");
    update();
}
void
MyWidgetI::tsave_slot()
{
    tpar.save("mesh.txt");
    update();
}
...
void
MyWidgetI::mload_slot()
{
    mpar.load("matrix.txt");
    update();
}
void
MyWidgetI::msave_slot()
{
