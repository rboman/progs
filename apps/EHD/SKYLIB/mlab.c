...
int mlab_vec(char *filename, char *id_txt, double *v, int n, 
             int nfile, int opt)
{
  int iop=0;
  int i;
...
int mlab_mat(char *filename, char *id_txt, double **v, int m, int n, 
             int nfile, int opt)
{
  int iop=0;
  int i,j;
...
int mlab_mat_mxn(char *filename, char *id_txt, int m, int n, 
             ...
             int type, int nfile, int opt)
{
  ...
  name = A->name;
  pre1 = (type==TDI_LU)? pre[1] : pre[0];
  pre2 = (type==TDI_LU)? pre[2] : pre[0];
  ...
  for(i=0;i<A->nsys;i++) {
    ...
    if(type==TDI_LU)
      fprintf(fich,"%s%s%s(%d,%d) = 1.0;\n",
              name,id_txt,pre1,i1,i1);
    fprintf(fich,"%s%s%s(%d,%d) = %20.15E;\n",
            name,id_txt,pre2,i1,i1,A->s[1][i]);
    if(i!=0)
      fprintf(fich,"%s%s%s(%d,%d) = %20.15E;\n",
              name,id_txt,pre1,i1,i1-1,A->s[2][i]);
    if(i!=A->nsys-1)
      fprintf(fich,"%s%s%s(%d,%d) = %20.15E;\n",
              name,id_txt,pre2,i1,i1+1,A->s[0][i]);
  }
