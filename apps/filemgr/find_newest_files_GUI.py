...
class Window(QWidget):
    def __init__(self, parent = None):
        ...
        inpframe = QFrame()
        inplayout = QGridLayout(); inplayout.setContentsMargins(0,0,0,0)      
        inpframe.setLayout(inplayout)

        folder = QLabel('Folder')
        self.folderEdit = QLineEdit()
        self.folderEdit.setText(os.getcwd())
        iconfile = os.path.join(os.path.dirname(__file__), '..','..','ico','boomy-foldersearch.png')
        ...
        button = QPushButton(self.tr("Go!")) ; butlayout.addWidget(button)
        button.pressed.connect(self.process)
        iconfile = os.path.join(os.path.dirname(__file__), '..','..','ico','boomy-play.png')
...
def main():
    ...
    iconfile = os.path.join(os.path.dirname(__file__), '..','..','ico','boomy-stats.png')
