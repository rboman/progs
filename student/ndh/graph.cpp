/*********************************************************************
 *                                                                   *
 *	      Travail N.D.H. : ElÇments aux frontiäres               *
 *            ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   	     *
 *	      Version C++    derniäre modif.: 30.11.96               *
 *                                                                   *
 *********************************************************************
...
void visu()
{
   ...
   else                       // palette Çcran (fond = noir).
   {  for(i=0;i<16;i++)
	 setrgbpalette(pal.colors[i],i*4,0,0);
      setrgbpalette(pal.colors[15],63,63,63);
   }
   if(calcul==1)              // pas d'affichage des T si
   {                          // pas de calculs effectuÇs
   find_minmax();
   dT=Tmax-Tmin;
   if(cartesien==1)
   {  for(i=0;i<density;i++)
      for(j=0;j<density;j++)
      {	 temp = a/density*i;     poly[0]=temp*zoom;
	 temp = a/density*(i+1); poly[2]=temp*zoom;
	 temp = a/range*j;       poly[1]=-temp*zoom;
	 temp = a/range*(j+1);   poly[3]=-temp*zoom;
	 if((probleme==1)&&(range==1))
	    color=(ncol-3)*(T[i][0]-Tmin)/dT+1;
	 else
	    color=(ncol-3)*(T[i][j]-Tmin)/dT+1;
	 if (maillag==1) setcolor(15); else setcolor(color);
	 ...
	 rectangle(-poly[0]+xo,-poly[1]+yo,-poly[2]+xo,-poly[3]+yo);
      ...
      for(i=0;i<density;i++)
      for(j=0;j<N;j++)
      {  temp = xf[j]/density*i;       poly[0]=temp*zoom+xo;
	 temp = xf[j]/density*(i+1);   poly[2]=temp*zoom+xo;
	 temp = xf[j+1]/density*(i+1); poly[4]=temp*zoom+xo;
	 temp = xf[j+1]/density*i;     poly[6]=temp*zoom+xo;
	 temp = yf[j]/density*i;       poly[1]=-temp*zoom+yo;
	 temp = yf[j]/density*(i+1);   poly[3]=-temp*zoom+yo;
	 temp = yf[j+1]/density*(i+1); poly[5]=-temp*zoom+yo;
	 temp = yf[j+1]/density*i;     poly[7]=-temp*zoom+yo;
	 if((probleme==1)&&(range==1))
	    color=(ncol-3)*(T[i][0]-Tmin)/dT+1;
	 else
	    color=(ncol-3)*(T[i][j]-Tmin)/dT+1;
   ...
   // dessine les ÇlÇments frontiäres:
   for(i=0; i<N; i++)
