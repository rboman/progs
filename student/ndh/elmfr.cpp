 ...
 *  Compilation : utiliser le modŠle 'large'.                        *
 *********************************************************************/
...
{  int i,j;
   ...
   {  j=N/4; N=4*j;
      ...
      for(i=0; i<=j; i++)
      {  xf[i]=a;                yf[i]=alpha[i];
	 xf[i+j]=alpha[j-i];     yf[i+j]=a;
	 xf[i+2*j]=-a;           yf[i+2*j]=alpha[j-i];
	 xf[i+3*j]=alpha[i];     yf[i+3*j]=-a; }
   }
   for(i=0; i<N; i++)
   {  xel[i]=(xf[i]+xf[i+1])/2;
      yel[i]=(yf[i]+yf[i+1])/2; }
...
// (Cette routine r‚soud le problŠme pos‚)
...
{  int i, j, i1, j1,t;
   ...
   {  // Cas du problŠme qcq. avec calculs optimis‚s.
      // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      cout << "\nPas de solution rapide pour un problŠme QCQ !\n<ESPACE>";
      getch(); }
   else
   {  time1=clock();    // on commence … compter le temps CPU.
      calcul=1;         // le calcul va ˆtre effectu‚.
      destroy_aux();    // lib‚ration de la m‚moire.
      cout << "\n\nCr‚ation des matrices H et G...";
      create_GH();
      cout << "Ok\nCalcul des matrices H et G...";
      if(type==1)
	 // Cas du problŠme non optimis‚:
	 // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 for(i=0; i<N;i++)
	    for(j=0;j<N;j++)
	       eval_GH(&(G[i][j]),&(H[i][j]),i,j,xel[i],yel[i]);
      else
      {  // Cas du problŠme optimis‚:
	 // ~~~~~~~~~~~~~~~~~~~~~~~~~
	 ...
	 { t=N/4;                             //     *****
      ...
      // Evaluation des T sur la frontiŠre et r‚solution du
      // systŠme par Gauss:
	 ...
	 mmv(N,H,u,alpha);
      ...
      if((probleme==1)&&(type==2)) range=1;
      else if((probleme==2)&&(type==2)) range=density;
      else range=N;
      if((probleme==2)&&(type==2)) cartesien=1; else cartesien=0;
      create_aux();

      // Calcul des points xb,yb o— va ˆtre ‚valu‚e la T.
	 ...
	 for(j1=0;j1<range;j1++)
	 {
	    ...
	    // Calcul de la solution du problŠme de Poisson
	    temp=0.0;
	    for(j=0;j<N;j++)
	       temp=temp+G1[j]*q[j]-H1[j]*u[j];
	    // Calcul de la solution du problŠme pos‚
	    r=sqrt(xb*xb+yb*yb);
   ...
   {  time1=clock(); // d‚but des calculs.
	 ...
	 {  if((probleme==1)&&(range==1))
	    ...
	    {  temp=0.0;
		  ...
		  temp=temp+(1.0/(i*i*i))
			   *pow(-1.0,(i-1)/2.0)
			   *(1-cosh((i*pi*yb)/(2.0*a))/cosh((i*pi)/2.0))
			   *cos((i*pi*xb)/(2.0*a));
...
void main()
{
   ...
   while(exit==0)
   {
      clrscr(); titre();
      cout << "\n\nProblŠme courant :";
      if(probleme==1) cout << " CERCLE de rayon a";
      else if(probleme==2) cout << " CARRE de c“t‚ a";
      ...
      choix=getch();
      switch(choix)
      {  case '1': {type=1; full_calcul();} break;
	 case '2': {type=2; full_calcul();} break;
	 case '3': input_data(); break;
	 case '4': load_data(); break;
	 case '5': visu(); break;
	 case '6': eval_Texact(); break;
	 case '7': save_Mfile(); break;
	 case '0': exit=1; break;
      }
   }
   clrscr();
