...
class Vararray {
    ...
    void resize(int) ;
	void swap(int a,int b){T tmp=data[a]; data[a]=data[b]; data[b]=tmp;};
 ...
 public:
    ...
    void remove_one_record(int i){
       if (debug) cerr <<"Vararray remove "<< this <<" element "<< i <<"";
       swap(i,last);
       last--;
    }
...
Vararray<T>& Vararray<T>::operator= (const Vararray<T>& a) {
  if (this != &a) {
	 ...
	 last  = a.last;
     data  = new T [maxsz+1] ;
     for (int i=0; i<=maxsz; ++i) data[i] = a.data[i];
  }
  return *this;
};
...
Vararray<T>::Vararray(const Vararray<T>& a) {
  ...
  data  = new T [maxsz+1] ;
  for (int i=0; i<=maxsz; ++i) data[i] = a.data[i];
  if(debug) cerr <<"New Vararray of size "<< maxsz
		 <<" pointer "<< this <<"-"<< data <<"\n";
};
...
template<class T>
Vararray<T>::~Vararray(){
  if(debug) cerr <<"Delete Vararray of size "<< maxsz
		 <<" pointer "<< this <<"-"<< data <<"\n";
  delete[] data;
};
...
void Vararray<T>::resize(int fit) {
  ...
  for(int i=0; i <= maxsz ; ++i) newdata[i] = data[i] ;
//  for(       ; i <=newsize; ++i) newdata[i] = 0 ;

  if(debug) cerr <<"Resizing Vararray to size "<< newsize
