...
simulation()
{
    ...
    for (i = 1; i <= 73; i++)
    {
        ...
        x1[i] =
            2 * atan((k2 + sqrt(k2 * k2 - (k3 + k1) * (k3 - k1))) / (k3 + k1));
    ...
    while (!kbhit())
        for (i = 1; i <= 73; i++)
        {
            ...
            line(x[3][i], y[3][i] - e * zoom * 350 / 480, x[3][i] + 10 * zoom,
                 y[3][i] - e * zoom * 350 / 480);
            line(x[3][i] - 0.5 * zoom, y[3][i], x[3][i] + 0.5 * zoom, y[3][i]);
            line(ox - 0.5 * zoom, oy - ya * zoom * 350 / 480, ox + 0.5 * zoom,
                 oy - ya * zoom * 350 / 480);
            for (j = 1; j < 73; j++)
