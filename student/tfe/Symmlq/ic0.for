      SUBROUTINE IC0 (NROW,A,IA,JA,L,IL,JL,JT,jt2,JI,DR,NZLMAX,
     $                INFO)
C
...
C
         ...
         DO 60 J = 1, I-1
            ZSUM = ZZERO
C
C     Iterate over the columns of row J, up to the diagonal.
C
            DO 50 KK = IL(J), IL(J+1)-1
               K = JL(KK)
C
C     If a common nonzero element, ZSUM += L(I,K) * L(J,K).
C
               IF (JT(K)) ZSUM = ZSUM + L(KK) * DR(K)
 50         CONTINUE
C
C     L(I,J) = ( A(I,J) - ZSUM ) / L(J,J).
C     If nonzero, add it to the running row sum.
C
            ZSUM = ( DR(J) - ZSUM ) * L(J)
...
C
 90   CONTINUE
        OPEN(UNIT=1,FILE='llt.m',STATUS='UNKNOWN')
        Do i=1,NROW
          write(1,*)'L(',i,',',i,')=',1.0D0/l(i),';'
          DO j=IL(i),IL(i+1)-1
            write(1,*)'L(',i,',',jl(j),')=',l(j),';'
          ENDDO
        ENDDO
        CLOSE(1)
C
