...
%        Calcul du Cn
      for n=1:20
         C(n)=0;
         for i=1:NZ
            C(n)=C(n)+1/(L*(bm(i)^2+H^2)+H)*exp(-(bm(i))^2*eta*(NT)*dt)/cos(bm(i)*L)*(-1)^(n+1)*bm(i)*(L/n/pi)^2*sin(bm(i)*L)/(1-(bm(i)*L/n/pi)^2);
         end
         C(n)=C(n)*(-2*Tf*H*2/L);
      end
...
%     Calcul somme sur n
      ua(j,k)=C0;
      for n=1:20
         ua(j,k)=ua(j,k)+C(n)*cos(n*pi*x(j)/L)*exp(-(n*pi/L)^2*eta*(t(k)-NT*dt));
      end
   end
end  
ua=flipud(ua);