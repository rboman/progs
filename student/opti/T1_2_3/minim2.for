...
C
C derniere modification : 09.02.97
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
...
C     -------------------------
      ...
      DIMENSION A(1),X(1),B(1),G(1),Z(1),ISA(*)
C
...
C
   2  CONTINUE
C
C        Calcul du gradient et mise a jour de f(x)
C        Projection du gradient (si necessaire)
C        -----------------------------------------
         CALL TLVECT (A,X,G,N)
...
C        ----------------------
         WRITE(1,*)'FCT(',ITER+1,')=',FCT,';'
         WRITE(1,*)'G(',ITER+1,')=',CONV,';'
         WRITE(1,*)'TAU(',ITER+1,')=',TAU,';'
         DO i=1,N
            WRITE(1,*)'X(',i,',',ITER+1,')=',X(i),';'
            WRITE(1,*)'Z(',i,',',ITER+1,')=',Z(i),';'
         ENDDO
C
         ITER=ITER+1
         ITER2=ITER2+1
...
C
         ...
         ELSE
            IF(ZMOD.LT.TOL) THEN
               ...
               DO i=1,N
                  G(i)=G(i)-B(i)
                  IF (G(i).gt.0) G(i)=ZERO
               ENDDO         
               ZMOD=DOTPRO(G,G,N)
               CONV=DSQRT(ZMOD)
               IF (CONV.LT.TOL) GOTO 99
            ENDIF
         ENDIF
C
