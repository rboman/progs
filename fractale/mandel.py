...
class Transf:
    ...
    def __init__(self, wx, wy, ox=0.0, oy=0.0, zoom=1.0):
        ...
        self.zoom = float(zoom)    # zoom factor (a logical length of 1 is 'zoom' pixel long)
    ...
    def run(self):
        ...
        for xe in range(0, self.transf.wx, self.inc):
            for ye in range(0, self.transf.wy, self.inc):
                ...
                n=0; xn=0.0; yn=0.0; xn2=0.0; yn2=0.0
...
class MandelWidget(QWidget):
    ...
    def __init__(self, parent = None):
        ...
        self.transf = Transf(self.width(), self.height(), ox=0.0, oy=0.0, zoom=80)
        self.thread = None
    ...
    def paintEvent(self, event):
        ...
        # zoom
        if self.showZoomBox:
            rect = self.checkAspect(self.mouse1.x(), self.mouse1.y(), self.mouse2.x(), self.mouse2.y())
            painter.setPen(Qt.white)
            painter.setBrush(QColor(0, 0, 0, 0)) # sinon herite de la brush precedente
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if self.mouse2 != self.mouse1:
                rect = self.checkAspect(self.mouse1.x(), self.mouse1.y(), self.mouse2.x(), self.mouse2.y())
                x1,y1 = self.transf.win2ax(rect.topLeft().x(), rect.topLeft().y())
                x2,y2 = self.transf.win2ax(rect.bottomRight().x(), rect.bottomRight().y())
                ox = (x1+x2)/2
                oy = (y1+y2)/2
    ...
    def closeEvent(self, event):
        ...
        settings = QSettings()
        settings.setValue("showCoords", QVariant(self.showcoords_action.isChecked()))
