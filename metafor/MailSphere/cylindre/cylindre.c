...
// PARAMETRES
   ...
   cyl_ouvert = 0 ; // 1 si ouvert, 0 si fermé 
   theta0 = 360. ; // ouverture en degré si ouvert
...
// inversion des rayons pour rext > rint
  ...
  if(rext < rint){
   ...
   if(cyl_creux==0){
     if(cyl_ouvert ==1){return ;}
     if(nbe % 4 != 0){
       nbe=(nbe/4+1)*4 ;
     }
   }

   if(cyl_ouvert==1){nbe2=(nbe+1) ;}
   else if(cyl_ouvert==0){nbe2=nbe ;}
...
//  impression des données du problème
	...
	fprintf(fp_out,"abrev '/rap' '%15.8E' ! Rapport de la diagonale du carre central au rayon exterieur \n",(rint/rext)) ;
     ...
     fprintf(fp_out,"abrev '/nbc' '%3d' ! Nombre de couches d'éléments (hors carre central si plein) \n",nbc) ;
...
// impression du .MAI
	 ...
	 for(ne=0; ne < nbe; ne++){
	   maille=maille+1 ;
	   ne2=ne+1 ;
	   if(cyl_ouvert==0 && ne==nbe-1){ne2=0 ; }
...
//   matrice souple
	 ...
	 if(ne != nbe2-1){fprintf(fp_out2,"I %8d \n",-3) ;}
