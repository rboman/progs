...
int main(int argc,char **argv)
{
   ...
   for(n=1;n<argc;n++) {
	 ...
	 if(checkparen(argv[n])==true) /* verifie les parentheses */
	   {
	     fileinp=fopen(argv[n],"r"); t=0; job=0; nblign=1;
	     while((buf=getc(fileinp))!=EOF) 
	       {
		 ascii=false;
		 if((buf>='A')&&(buf<='z')) {buf=toupper(buf);ascii=true;}
		 if(buf=='\n')nblign++;
		 
		 if((newligne==true)&&(buf=='C')) waitcom=true;
		 newligne=false;
		 if(buf=='\n') 
		   {newligne=true; if(waitcom==true) waitcom=false;}
		 ...
		 if(waitcom==false){
		   ...
		   switch(job)
			 ...
			 if(paren<0) 
		       ...
		       if(buf=='(') paren++;
		       if((buf==',')&&(paren==0)) vars++;
		       if(ascii==true) navar=1; /* on a trouve des lettres */
		       if((buf>='0')&&(buf<='9')) navar=1;
		       } break;
		     } /* endswitch */
		 } /* test commentaires */
	       } /* endwhile */
   ...
   for(n=1;n<argc;n++) {
	 ...
	 case 1:
	   fileinp=fopen(argv[n],"r"); t=0; job=0; nblign=0;
	   while((buf=getc(fileinp))!=EOF) 
	     {
	       ascii=false;
	       if(buf=='\n')nblign++;
		   ...
		   case 2:  /* cherche le nom de la SUBROUTINE */
			 ...
			 for(i=0;i<nbsub;i++)
			   {    if(strcmp(temptxt,nextsub->nom)==0)
			     {flag=1; nextsub->nb_appels++; i=nbsub;}
			   else
                             nextsub=nextsub->next;
			   }
			 paren=0; /* niv de parenthese */
			 ...
			 job=3;
			 if(buf=='(') job=4;
			 if(flag==0) job=0; /* Routine pas dans liste (A CONT) */
		       }
		     } break;
		   case 3:  /* cherche le '(' */
		     { if(buf=='(') job=4;
			 ...
			 {   if((buf!=' ')&&(buf!='/')&&(buf!='\n'))
			   ...
			   { /* trouve !*/
				 ...
				 if(templen==tempt->lnom)
				   if(strncmp(temptxt,tempt->nom,templen)==0) t=i;
				 if(t==-1) tempt=tempt->next;
			       }
			     if(t!=-1)
			 ...
			 if(npos==6)
			   {  if(buf!=' ') job=4;
				 ...
				 {  if(vars!=tempt->nb_vars)
				   ...
				   {if(nop2==true) printf("COMMON %s dans %s ......ok\n",tempt->nom,argv[n]);}
				 }
			       job=0; t=0;
			     }
			   }
...
int checkfile(char *file) 
{
   ...
   /*   for(i=0;i<l-2;i++)*/
   if(strcmp(file+l1,ext1)==0) return(1);
   if(strcmp(file+l2,ext2)==0) return(1);
   if(strcmp(file+l3,ext3)==0) return(2);
   if(strcmp(file+l4,ext4)==0) return(2);
   return(0);
}
...
{  puts("\n--------------------------------------\n");
   ...
   puts("--------------------------------------\n");
   puts("compiled : "__DATE__ " at "  __TIME__);
   printf("\n%s\n",vcid);
}
...
{   
    ...
    {   if((buf>='A')&&(buf<='z')) buf=toupper(buf);
        if((newlig==true)&&(buf=='C')) waitc=true;
        newlig=false;
        if(buf=='\n') 
	  {newlig=true;if(waitc==true) waitc=false;}
        if(waitc==false)
        {    if(texte==false)
             {  if(buf=='(') paren++;
	        if(buf==')') paren--; }
             if(buf=='\'') 
                if(texte==false) texte=true; else texte=false;
        }
    }
    fclose(fichpt);
    if(paren==0) return(true); else return(false);
}
