...
SphereBAD::build()
{
    ...
    int i, ii, j, k, l, n2, nelemtour, nelemext, face, taille, **maille,
        **posface, level;

    double rayonini, rayonext, **alpha, **beta, alpha1, beta1, x0, x1, x2,
        centre[3], pi, rayon, d1, d2, d3, pres, eps;
        ...
        for (face = 1; face <= 1; face++)
        {
            if (face == 1)
            {
                ...
                for (k = 0; k <= nelemtour; k++)
                {
                    for (l = 0; l <= nelemtour; l++)
                    {
                        ...
                        alpha1 = acos(
                            1 / sqrt(1 + sin(beta[k][l]) * sin(beta[k][l])));
            ...
            if (face == 1)
            {
                for (k = 0; k <= nelemtour; k++)
                {
                    for (l = 0; l <= nelemtour; l++)
                    {
                        x0 = rayon * cos(alpha[k][l]) * sin(beta[k][l]) +
                             centre[0];
                        x1 = rayon * sin(alpha[k][l]) + centre[1];
                        x2 = rayon * cos(alpha[k][l]) * cos(beta[k][l]) +
                             centre[2];
    ...
    for (ii = 1; ii <= nelemext * n2; ii++)
    {
        ...
        insertvtkcell(ugrid, maille[ii][0], maille[ii][1], maille[ii][2],
                      maille[ii][3], maille[ii][4], maille[ii][5],
                      maille[ii][6], maille[ii][7]);
    }
