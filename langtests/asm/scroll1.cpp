...
*                      Scrolling 2  (C++ & Asm)                        *
*                                                                      *
* . M‚morisation des caractŠres du DOS                  	       *
...
{" Premier scroll !!! ... ce programme est ‚crit en C++ et se passe des drivers BGI ...  ",
 " Les caractŠres du DOS sont simplement m‚moris‚s et affich‚s avec un pas de deux pixels.",
 ...
 " Prochaine ‚tape : le scrolling sinuso‹dal et tridimensionnel ... ",
 " ---------------------------------------",
 " ",
 " ",
 " " };

uc lettre[107][8];                     /* Data caractŠres 		    */
...
void aff_char(int x, int y, uc num, uc col1, uc col2,uc factor,uc effect)
{
   ...
   for(int i=0; i<8; i++)
      for(int j=0; j<8; j++)
      {
	 if(effect&1) col3=col1+j;
	 if(effect&(1<<1)) col4=col2+j;
...
/* Affiche une tranche de caractŠre (->scrolling) */
...
{                                    /*        = 2 : cisaillement           */
   ...
   if(effect==2)
   {
      ...
      for(t=0;t<319;t++)
      {
	 if(c==1) {waitretrace(); c=0;} else c=1;
	 for(i=0; i<199; i+=2)
	    { putpixel(t,i,0);
...
void main()
{
   ...
   while(!out)
   {
      ...
      waitretrace();
      pos++;
      if(pos==8) {pos=0; texte[nol]++;}
      if(*texte[nol]==0)
      { texte[nol]=debut; nol++;
	if(nol>MAXLIGNES) nol=0;
	debut=texte[nol];
      }
      if(kbhit())
	{
	  ...
	  frappe=getch();
	  if(frappe==' ') out=1;
	  if((frappe==U_arrow)&&(jt[cur]>0)) jt[cur]--;
	  if((frappe==L_arrow)&&(it[cur]>0)) it[cur]--;
	  if((frappe==R_arrow)&&(it[cur]<15)) it[cur]++;
	  if((frappe==D_arrow)&&(jt[cur]<15)) jt[cur]++;
	  if(frappe==ENTER)
	     if(cur==0)
		{ cur=1; aff_txt_ln(24,4,"background color",75,0,1,1); }
	     else
		{ cur=0; aff_txt_ln(24,4,"text color      ",75,0,1,1); }
	  box(it[0]*10, jt[0]*10, (it[0]+1)*10, (jt[0]+1)*10,4);
	  box(it[1]*10, jt[1]*10, (it[1]+1)*10, (jt[1]+1)*10,3);
	}
