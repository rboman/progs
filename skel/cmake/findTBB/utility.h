/*
    ...
    This file is part of Threading Building Blocks. Threading Building Blocks is
   free software; you can redistribute it and/or modify it under the terms of
   the GNU General Public License version 2  as  published  by  the  Free
   Software Foundation.  Threading Building Blocks is distributed in the hope
   that it will be useful, but WITHOUT ANY WARRANTY; without even the implied
   warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See  the GNU
   General Public License for more details.   You should have received a copy of
    the  GNU General Public License along with Threading Building Blocks; if
...
#include <iostream>
#include <cstdlib>
// TBB headers should not be used, as some examples may need to be built without
// TBB.
...
#if ((__GNUC__ * 100 + __GNUC_MINOR__ >= 404 && __GXX_EXPERIMENTAL_CXX0X__) || \
     _MSC_VER >= 1600) &&                                                      \
...
// std::unique_ptr is unavailable, so suppress std::auto_prt<> deprecation
// warning
...
template <typename T>
void
swap(std::auto_ptr<T> &ptr1, std::auto_ptr<T> &ptr2)
{
    std::auto_ptr<T> tmp;
    tmp = ptr2;
    ptr2 = ptr1;
...
dest_type &
string_to(std::string const &s, dest_type &result)
{
    std::stringstream stream(s);
    stream >> result;
    if ((!stream) || (stream.fail()))
    {
        throw std::invalid_argument("error converting string '" +
                                    std::string(s) + "'");
    }
    return result;
}
...
public:
    ...
    void parse_and_store(const std::string &s)
    {
        ...
        if (validating_function)
        {
            ...
            {
                std::stringstream str;
                str << "'" << target << "' is invalid value for argument '"
                    << name << "'";
                throw std::invalid_argument(str.str());
            }
        }
    ...
    {
        {
            ...
            for (std::size_t j = 1; j < argc; j++)
            {
                ...
                struct throw_unknown_parameter
                {
                    static void _(std::string const &location)
                    {
                        throw std::invalid_argument(
                            std::string("unknown parameter starting at:'") +
                            location + "'");
                    }
                };
                ...
                else
                {
                    ...
                    else if (current_positional_index <
                             positional_arg_names.size())
                    {
                        std::stringstream str(argv[j]);
                        args_map_type::iterator found_positional_arg =
                            args_map.find(positional_arg_names.at(
                        ...
                        {
                            throw std::logic_error("positional_arg_names and "
                                                   "args_map are out of sync");
                        }
                ...
                {
                    throw std::invalid_argument(
                        std::string("several values specified for: '") +
                        pa->name() + "' argument");
                }
...
int
step_function_plus(int previous, double step)
{
    return static_cast<int>(previous + step);
}
int
step_function_multiply(int previous, double multiply)
{
    return static_cast<int>(previous * multiply);
}
// "Power-of-2 ladder": nsteps is the desired number of steps between any
// subsequent powers of 2. The actual step is the quotient of the nearest
// smaller power of 2 divided by that number (but at least 1). E.g., '1:32:#4'
// means 1,2,3,4,5,6,7,8,10,12,14,16,20,24,28,32
int
step_function_power2_ladder(int previous, double nsteps)
{
    ...
    while (int shifted = prev_power2 >> rshift)
    { // shift the value right; while the result is non-zero,
        prev_power2 |= shifted; //   add the bits set in 'shifted';
        rshift <<= 1; //   double the shift, as twice as many top bits are set;
    }                 // repeat.
    ++prev_power2; // all low bits set; now it's just one less than the desired
                   // power of 2
...
public:
    step_function_descriptor(char a_mnemonic, step_function_ptr_type a_function)
        : mnemonic(a_mnemonic), function(a_function)
    {
    }
...
struct thread_range_step
{
    ...
    thread_range_step(step_function_ptr_type step_function_,
                      double step_function_argument_)
        : step_function(step_function_),
          step_function_argument(step_function_argument_)
    {
        if (!step_function_)
            throw std::invalid_argument(
                "step_function for thread range step should not be NULL");
    }
                                    ...
                                    thread_range_step &step)
    {
        ...
        {
            throw std::invalid_argument(
                "the argument of # should be a power of 2");
        }
...
struct thread_number_range
{
    ...
    thread_number_range(
        int (*auto_number_of_threads_)(), int low_ = 1, int high_ = -1,
        internal::thread_range_step step_ = internal::thread_range_step(
            internal::step_function_power2_ladder, 4))
        : auto_number_of_threads(auto_number_of_threads_), first(low_),
          last((high_ > -1) ? high_ : auto_number_of_threads_()), step(step_)
    {
        if (first < 0)
        {
    ...
    friend std::istream &operator>>(std::istream &i, thread_number_range &range)
    {
        try
        {
            ...
            struct string_to_number_of_threads
            {
                ...
                {
                    return (value == "auto") ? auto_value
                                             : internal::string_to<int>(value);
                }
            };
            string_to_number_of_threads string_to_number_of_threads(
                range.auto_number_of_threads());
            int low, high;
            ...
            else
            {
                ...
                low = string_to_number_of_threads(
                    std::string(s, 0, colon)); // not copying the colon
                high = string_to_number_of_threads(std::string(
                    s, colon + 1,
                    second_colon - (colon + 1))); // not copying the colons
                if (second_colon != std::string::npos)
                {
                    internal::string_to(std::string(s, second_colon + 1),
                                        range.step);
                }
            }
    ...
    friend std::ostream &operator<<(std::ostream &o,
                                    thread_number_range const &range)
    {
        using namespace internal;
        size_t i = 0;
        for (;
             i < array_length(step_function_descriptors) &&
        ...
        {
            throw std::invalid_argument(
                "unknown step function for thread range");
        }
        o << range.first << ":" << range.last << ":"
          << step_function_descriptors[i].mnemonic
...
static const char *thread_number_range_desc =
    ...
    "\n\twhere low and optional high are non-negative integers or 'auto' for "
    "the default choice,"
    "\n\tand optional step expression specifies how thread numbers are chosen "
    "within the range."
    "\n\tSee examples/common/index.html for detailed description.";

inline void
report_elapsed_time(double seconds)
{
    std::cout << "elapsed time : " << seconds << " seconds \n";
}

inline void
parse_cli_arguments(int argc, const char *argv[],
...
inline void
parse_cli_arguments(int argc, char *argv[], utility::cli_argument_pack cli_pack)
{
