...
// Workaround for ICC 11.0 not finding __sync_fetch_and_add_4 on some of the
// Linux platforms.
...
CountOccurrences(int nthreads)
{
    ...
    if (!silent)
        printf("total = %d  unique = %u  time = %g\n", n,
               unsigned(table.size()), (t1 - t0).seconds());
}
...
Sound Consonants[] = {{"r", {483, 1414, 1110}},
                      {"n", {312, 1548, 1114}},
                      {"t", {363, 1653, 251}},
                      {"l", {424, 1341, 489}},
                      {"c", {734, 735, 260}},
...
int
CountRateSum(Sound sounds[], const int num, const int part)
{
    int sum = 0;
    for (int i = 0; i < num; i++)
...
const char *
GetLetters(int type, const int part)
{
    Sound *sounds;
    int rate, i = 0;
    if (type & 1)
...
CreateData()
{
    ...
    helloworld += ", " + Data[1] + " " + Data[2] + " " + Data[3] + " " +
                  Data[4] + " " + Data[5];
    if (!silent)
...
main(int argc, char *argv[])
{
    try
    {
        ...
        // The 1st argument is the function to obtain 'auto' value; the 2nd is
        // the default value The example interprets 0 threads as "run serially,
        // then fully subscribed"
        utility::thread_number_range threads(
            tbb::task_scheduler_init::default_num_threads, 0);

        utility::parse_cli_arguments(
            argc, argv,
            utility::cli_argument_pack()
                //"-h" option for displaying help is present implicitly
                .positional_arg(threads, "n-of-threads",
        ...
        { // Number of threads wasn't set explicitly. Run serial and parallel
          // version
            { // serial run
                if (!silent)
                    printf("serial run   ");
        ...
        utility::report_elapsed_time(
            (tbb::tick_count::now() - mainStartTime).seconds());
