...
"""Script used to build&update Metafor on Windows, Linux and macOS
"""
...
def main(repos, opts):
    ...
    # remove Metafor binaries
    if not os.path.isdir('oo_metaB'):
        print('oo_metaB folder is missing!')
        build_required = True
        # TODO: we should check whether binaries are present

    if not build_required:
        print('=> build seems NOT to be required')
    ...
    if build_required:
        ...
        # build using incredibuild if present
...
if __name__ == "__main__":
    ...
    if opts['build_type']['value']=='full':
        ...
        repos.append(vrs.GITRepo('keygen', 'git@gitlab.uliege.be:am-dept/keygen.git'))
